...
### `api_os.py` (inclui Reparos catálogo padrão) ✅ extraído em 2026-08-07 — 12º e último domínio
- **Helpers (10):** `_os_row_to_dict`, `_ordem_lista_por_id_desc`, `_parse_checklist_json`, `_checklist_status`, `_serialize_checklist`, `_resolve_public_base_url`, `_enriquecer_checklist_urls`, `_buscar_checklist_por_os`, `_buscar_checklist_por_token`, `_garantir_checklist_os` (+ `err`/`ok`/`usuario_logado`/`usuario_admin`/`_texto_limpo_local` de `fluxoly_api_helpers.py`). **Correção da Discovery Local:** `carregar_config_mercadophone`/`atualizar_runtime_mercadophone` não são helpers locais — são importadas diretamente de `fluxoly_mercadophone.py` (único ponto de todo `api_os.py` fora do padrão puro `deps[...]`), usadas só por `listar_ordens()` para filtrar OS antigas da integração
- **Deps (35, não 32):** `conectar`, `normalizar_status_os`, `status_finalizado`, `status_cancelado`, `status_aberto`, `calcular_faturamento_os`, `calcular_lucro_os`, `carregar_os_com_relacoes`, `validar_reparo_ids`, `vendedor_valido`, `salvar_reparos_os`, `modelo_compativel`, `consumir_peca_da_os`, `adicionar_peca_os_sem_consumir`, `devolver_pecas_da_os`, `obter_reparos_por_os`, `buscar_reparo_ids_da_os`, `resolver_garantias_reparo`, `gravar_garantias_reparo`, `buscar_linhas_com_garantia_da_os`, `zerar_garantia_reparo`, `buscar_garantia_reparo`, `corrigir_garantia_reparo`, `buscar_historico_garantia_reparo`, `obter_tipo_garantia`, `registrar_log_auditoria`, `modelo_para_os`, `normalizar_imei`, `texto_reparos_os`, `parse_data_ymd`, `vendedores`, `mercado_phone_runtime_config`, `public_base_url`, `integrations_config_path`, `carregar_configuracoes_integracoes`. **Correção da Discovery Local:** a matriz Phase 1 não capturava `public_base_url`, `integrations_config_path`, `carregar_configuracoes_integracoes` (mesmo padrão de correção já visto em quase toda extração desta sprint)
- **Achado de dep morta pré-existente (não introduzida por esta extração):** `status_em_andamento`/`status_aguardando_peca` (`STATUS_EM_ANDAMENTO`/`STATUS_AGUARDANDO_PECA`) estavam no dict de `create_api_blueprint` em `app.py` mas nunca eram lidas por nenhuma rota de OS nem de nenhum domínio já extraído — confirmado via `git show` do arquivo original antes desta extração. Removidas do dict junto desta extração, mesmo destino de `extrair_reparo_ids`/`garantir_pasta_backup_google_drive` (imports órfãos já mortos antes desta sessão, removidos por `ruff check --fix`)
- **Bug de transcrição encontrado e corrigido durante a extração (não existia no código original):** um corte de linha incorreto durante a migração via `sed` cortou o `return ok()` final de `deletar_reparo()`, fazendo a rota retornar `None` (500) no caminho de sucesso. Pego pelo smoke test manual do catálogo de Reparos (KI-033) antes de finalizar — corrigido e revalidado com diff linha a linha contra o bloco original, confirmando zero divergência
- **Serviços:** `fluxoly_mercadophone` (import direto, ver achado de helpers acima). As demais dependências de domínio (`fluxoly_os`, garantia por reparo, auditoria) chegam via `deps`, não import direto
- **Estado final de `fluxoly_blueprints_api.py`:** reduzido a 34 linhas — só os 2 helpers mortos de Estoque (`_slug_estoque`/`_gerar_sku_estoque`, KI-032) e `create_api_blueprint(deps)` retornando um `Blueprint` sem nenhuma rota registrada. `app.py` ainda registra esse blueprint vazio (`create_api_blueprint({})`) — remoção do arquivo/registro é decisão de Phase 3 (Cleanup), fora do escopo desta extração
