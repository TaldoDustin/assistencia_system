...
- Modo de demonstração comercial (`demo/commercial-preview`): `frontend/src/pages/Clientes.jsx` — primeira tela real para o domínio Clientes (Sprint P0.1 só tinha backend, sem frontend), CRUD completo contra `/api/clientes`. `Vendas.jsx`, `Financeiro.jsx` e `Insights.jsx` — telas de preview comercial com dados fictícios (`frontend/src/lib/demoData.js`) e badge `PreviewBadge` (`frontend/src/components/ui/preview-badge.jsx`), para nunca apresentar dado fake como real numa demonstração
- `Dashboard.jsx` ampliado com 3 dados 100% reais que o backend já calculava mas o frontend nunca exibia (`gasto_total_estoque`, `resumo_por_vendedor`, total de `/api/clientes`) e uma seção "Explore mais" com atalhos para Vendas/Financeiro/Fluxoly Insights (todos com `PreviewBadge`) — o Dashboard não falava mais só do módulo de Assistência, sem misturar dado fake nos KPIs reais
...
- Rota `/compras` duplicada (`ShoppingList.jsx` vs `Compras.jsx`) resolvida: `Compras.jsx` lia uma chave (`data.compras`) que a API real nunca retorna, então a lista sempre aparecia vazia e o botão de editar status nunca aparecia na prática — não era possível editar status por nenhum dos dois caminhos (KI-018, achado relatado pelo usuário testando a demo). Removidos `Compras.jsx` e `EditShoppingItemModal.jsx` (mortos); adicionada edição de status em `ShoppingList.jsx` via `PATCH /shopping-list/<id>/status` (endpoint já existente e validado)
