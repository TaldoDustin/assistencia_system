...
- `docs/company/SALES_DECK.md`, `docs/company/DEMO_SCRIPT.md`, `docs/company/FAQ_COMERCIAL.md` — material comercial (apresentação, roteiro de demo cronometrado, respostas a objeções), fundamentado em `BRAND_IDENTITY.md`/`VISION.md`/`PRODUCT_REQUIREMENTS.md`/`RELEASE_STRATEGY.md` já existentes, sem inventar posicionamento novo. Roadmap da seção 8 do Sales Deck corrige a distinção binária "pronto/não pronto" proposta inicialmente para 3 níveis (produção real / prévia comercial / visão de longo prazo), evitando superestimar o que já existe (ex.: Fluxoly Insights é hoje uma prévia visual, não um motor de IA real)
