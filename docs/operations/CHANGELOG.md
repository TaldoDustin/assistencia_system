...
- Modo de demonstração comercial (`demo/commercial-preview`): `frontend/src/pages/Clientes.jsx` — primeira tela real para o domínio Clientes (Sprint P0.1 só tinha backend, sem frontend), CRUD completo contra `/api/clientes`. `Vendas.jsx`, `Financeiro.jsx` e `Insights.jsx` — telas de preview comercial com dados fictícios (`frontend/src/lib/demoData.js`) e badge `PreviewBadge` (`frontend/src/components/ui/preview-badge.jsx`), para nunca apresentar dado fake como real numa demonstração. Dashboard real não foi alterado
