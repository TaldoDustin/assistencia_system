...
| TD-18 | **TD-01 Phase 3 — Cleanup.** Remover `fluxoly_blueprints_api.py` (só contém `_slug_estoque`/`_gerar_sku_estoque`, KI-032, código morto sem consumidor) e o registro `app.register_blueprint(create_api_blueprint({}))` em `app.py`, resolvendo KI-032 no processo. Deliberadamente não feito junto da extração de OS (2026-08-07) — mesma regra de sempre: uma mudança por vez, não misturar refatoração estrutural com limpeza | Baixo (cosmético, sem risco funcional — arquivo não registra nenhuma rota) | Baixa — sem prazo, backlog aberto por decisão do usuário ao encerrar a TD-01 |
| TD-02 | **`app.py` acumula inicialização, DB e lógica misturadas.** EM ANDAMENTO desde 2026-08-07, ver `docs/operations/SPRINTS/SPRINT_TD02_BOOTSTRAP_APP.md`. Fatia 1/4 (`fluxoly_config.py`, config/env/paths) e Fatia 2/4 (`fluxoly_app_security.py`, CORS/security headers) concluídas em 2026-08-07. Fatia 3/4 (`fluxoly_blueprint_registry.py`, registry dos 20 blueprints) concluída em 2026-08-08 — `app.py` 2.341 → 1.923 linhas. Falta Fatia 4/4 (webhook MercadoPhone → `api_mercadophone.py`) | Alto    | Alta — em execução |
...
| ~~TD-17~~ | ~~Crescimento do bootstrap de `app.py` (candidata a TD-02).~~ Achado do Architecture Checkpoint pós-MercadoPhone (TD-01 Phase 2, 9/12 domínios, 2026-08-06) — 20 `app.register_blueprint()` inline sem registry/factory. **Endereçado pela TD-02 Fatia 3 (2026-08-08):** `fluxoly_blueprint_registry.py::registrar_blueprints(app, runtime)` centraliza as 20 chamadas numa tabela declarativa. Não fechado como "Resolvido" formal aqui porque a TD-02 como sprint ainda não terminou (falta Fatia 4) — ver TD-02 acima | Baixo hoje (app.py funciona, cresce previsivelmente); médio a longo prazo se a TD-01 continuar sem uma TD-02 para conter o bootstrap | Endereçado — TD-02 Fatia 3 concluída, Fatia 4 restante não afeta este achado especificamente |
