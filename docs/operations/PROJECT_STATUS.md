...
| R-10 | Workflow `CI` nunca concluiu com sucesso em `main` desde que existe (105/105 runs falhos, 2026-07-07–2026-07-27) — job `Frontend Quality` (ESLint) vermelho por 3 causas diferentes ao longo do tempo, `Frontend Build` nunca roda (KI-026) | Alta | Alto (nenhum merge recente foi de fato validado pelo build/lint do frontend) | Nenhuma — `Lint` (Ruff)/`Backend Tests`/`Coverage` não são afetados e mascaram o problema em qualquer olhar rápido ao CI |
