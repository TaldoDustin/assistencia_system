...
| Dívida técnica     | Alta                            |
| Segurança          | Melhor — Sprint Segurança 1.0 (2026-07-25) fechou os P0/P1 do scan Aikido: fallback inseguro de `FLASK_SECRET_KEY` corrigido, autorização de OS/Estoque por perfil, headers HTTP (CSP/X-Frame-Options), Docker non-root, `persist-credentials`, gunicorn/deps JS atualizados. Falta 1 ação manual: rotacionar `FLASK_SECRET_KEY` em produção (Render) — ver `docs/security/SECURITY_AUDIT_2026-07.md` |
