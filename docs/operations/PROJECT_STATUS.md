...
| R-10 | Workflow `CI` nunca concluiu com sucesso em `main` desde que existe (84/84 runs falhos confirmados via `total_count` da API, 2026-07-07–2026-07-27) — job `Frontend Quality` (ESLint) vermelho por 3 causas diferentes ao longo do tempo, `Frontend Build` nunca roda (KI-026) | Alta | Alto (nenhum merge recente foi de fato validado pelo build/lint do frontend) | Nenhuma — `Lint` (Ruff)/`Backend Tests`/`Coverage` não são afetados e mascaram o problema em qualquer olhar rápido ao CI |
| R-11 | `main` não tem proteção de branch configurada no GitHub — confirmado via `gh api repos/.../branches/main/protection` retornando `404 Branch not protected` (sem revisão obrigatória, sem status check obrigatório, sem bloqueio de force-push). O CI (R-10) nunca funcionou como gate de merge: qualquer push/PR podia mergear em `main` independente do resultado — causa raiz de R-10 ter passado 84 execuções despercebido | Alta | Alto (nenhuma garantia estrutural de que código quebrado fique fora de `main`; depende inteiramente de disciplina manual) | Nenhuma — requer decisão do usuário (CTO) para configurar (ação em configuração de repositório, fora do escopo de um commit de código) |
