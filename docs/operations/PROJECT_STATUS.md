...
|------|------------------------------------------------------------------------|---------|------------|
| TD-01 | `fluxoly_blueprints_api.py` reduzido de ~130KB/3.368 linhas/70 rotas para 911 bytes/34 linhas/0 rotas. Sprint própria iniciada em 2026-08-04, **Phase 2 (Extração Incremental) CONCLUÍDA em 2026-08-07 — 12/12 domínios extraídos** (Shopping List, Garantias, Custos Operacionais, Preços, Usuários, Auth, Backup, Relatórios, MercadoPhone, Sistema, Estoque, OS+Reparos — 12 blueprints `api_*.py`). `fluxoly_blueprints_api.py` só contém código morto (KI-032) e um blueprint vazio, registrado por `app.py`. **Phase 3 (Cleanup) não iniciada** — decisão de encerramento formal da TD-01 e remoção do arquivo pendente do usuário. Ver `docs/operations/SPRINTS/SPRINT_TD01_MODULARIZACAO_API.md` (Architecture Checkpoint Final) e `docs/engineering/API_DEPENDENCY_MATRIX.md` | Alto    | Média (Phase 2 done; Phase 3 é limpeza, não estrutural) |
...
| TD-17 | **Crescimento do bootstrap de `app.py` (candidata a TD-02).** Achado do Architecture Checkpoint pós-MercadoPhone (TD-01 Phase 2, 9/12 domínios, 2026-08-06): cada domínio extraído reduz `fluxoly_blueprints_api.py`, mas move complexidade para `app.py` — hoje 2.431 linhas/100KB/17 `app.register_blueprint()`, um dict de `deps` distinto por blueprint. Padrão saudável enquanto durar a TD-01 (mantém `deps` explícito e testável por domínio), mas não escala indefinidamente: `app.py` cresce a cada extração, sem teto natural. Candidato a uma TD-02 futura — registry de blueprints, factories de `deps` por domínio, ou módulo de bootstrap dedicado — para `app.py` deixar de crescer linearmente com o número de domínios. Deliberadamente fora do escopo da TD-01 (misturaria decomposição do monólito com refatoração do bootstrap) | Baixo hoje (app.py funciona, cresce previsivelmente); médio a longo prazo se a TD-01 continuar sem uma TD-02 para conter o bootstrap | Baixa — sem prazo; reavaliar ao concluir a TD-01 (12/12 domínios), quando o tamanho final de `app.py` estiver claro |
