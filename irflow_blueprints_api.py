...
def create_api_blueprint(deps):
    ...
    def shopping_list():
        ...
        cursor.execute(
            """
            SELECT s.id, COALESCE(s.produto, ''), s.os_id, COALESCE(s.quantidade, 1), COALESCE(s.status, ''), COALESCE(o.id_externo_integracao, o.id)
            FROM compras s
            LEFT JOIN os o ON o.id = s.os_id
            ORDER BY s.id DESC
            """
        )
        rows = cursor.fetchall()
        ...
        for r in rows:
            items.append({
                ...
                "produto": r[1],
                "os_id": r[2],
                "quantidade": int(r[3] or 1),
                "status": r[4] or "",
                "os_numero": r[5] or "",
            })
        return ok(compras=items)
    ...
    def shopping_create():
        ...
        os_id = body.get("os_id")
        quantidade = int(body.get("quantidade") or 1)
        status = (body.get("status") or "").strip() or "PENDENTE"
        if not produto:
            return err("Produto obrigatório")
        conn = conectar()
        cursor = conn.cursor()
        agora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute("INSERT INTO compras (produto, os_id, quantidade, status, criado_em, atualizado_em) VALUES (?,?,?,?,?,?)",
                       (produto, os_id, quantidade, status, agora, agora))
    ...
    def shopping_update(item_id):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("UPDATE compras SET os_id=?, quantidade=?, status=?, atualizado_em=? WHERE id=?",
                           (os_id, quantidade, status, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), item_id))
            conn.commit()
            cursor.execute("SELECT id, produto, os_id, quantidade, status FROM compras WHERE id=?", (item_id,))
            row = cursor.fetchone()
            conn.close()
            if not row:
                return err("Item não encontrado", 404)
            return ok(item={"id": row[0], "produto": row[1], "os_id": row[2], "quantidade": row[3], "status": row[4]})
        except Exception as exc:
            conn.rollback()
            conn.close()
            return err(str(exc))
    ...
    def shopping_delete(item_id):
        ...
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM compras WHERE id=?", (item_id,))
        conn.commit()
        conn.close()
