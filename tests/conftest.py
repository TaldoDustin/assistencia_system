...
def criar_item_estoque():
    ...
    def _criar(modelo="iPhone 13", quantidade=5, valor=50.0, descricao="Peca Teste"):
        conn = _app.conectar()
        try:
            cursor = conn.cursor()
            cursor.execute(
                ...
                VALUES (?,?,?,?,?,?,?,?)
                """,
                (descricao, valor, "Fornecedor Teste", quantidade, modelo, f"SKU-{uuid.uuid4().hex[:8]}", "Tela", "Padrao"),
            )
            conn.commit()
